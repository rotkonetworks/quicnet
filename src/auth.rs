...
pub async fn handshake_client(conn: &Connection, identity: &Identity) -> Result<PeerId> {
    ...
    verifying_key
        .verify(&our_challenge, &Signature::from_bytes(&server_sig))
        .map_err(|_| anyhow::anyhow!("server auth failed"))?;

    Ok(PeerId::from_public_key(&server_pubkey))
}

pub async fn handshake_server(conn: &Connection, identity: &Identity) -> Result<PeerId> {
    ...
    verifying_key
        .verify(&challenge, &Signature::from_bytes(&client_sig))
